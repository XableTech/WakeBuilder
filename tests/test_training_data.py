...
def test_data_preparation():
    ...
    print(f"  Hard negatives: ~{int(config.target_positive_samples * config.hard_negative_ratio)}")
    print(f"  Real negatives: ~{int(config.target_positive_samples * config.negative_ratio)}")
    expected_total = config.target_positive_samples * (1 + config.hard_negative_ratio + config.negative_ratio)
    ...
    try:
        ...
        if abs(pos_ratio - 1.0) <= tolerance:
            ...
            print(f"  [WARNING] Positive samples outside expected range (tolerance: {tolerance*100}%)")
