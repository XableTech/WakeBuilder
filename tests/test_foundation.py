...
def test_audio_normalization():
    ...
    print(f"Normalized audio: shape={normalized.shape}, range=[{normalized.min():.3f}, {normalized.max():.3f}]")
...
def test_synthetic_wake_word():
    ...
    print(f"Synthetic wake word:")
    print(f"  - Duration: {duration}s")
    print(f"  - Sample rate: {sample_rate}Hz")
    print(f"  - Frequency components: 200Hz, 800Hz, 1600Hz")
