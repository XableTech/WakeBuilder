...
class TemporalConvBlock(nn.Module):
    ...
    def __init__(self, dim: int, kernel_size: int = 3, dilation: int = 1):
        ...
        self.conv = nn.Sequential(
            ...
            nn.GroupNorm(
                1, dim
            ),  # GroupNorm with 1 group = LayerNorm, works with batch size 1
            nn.GELU(),
            nn.Conv1d(dim, dim, kernel_size, padding=padding, dilation=dilation),
            nn.GroupNorm(1, dim),
        )
...
class ASTWakeWordModel(nn.Module):
    ...
    def __init__(
        ...
        logger.info(
            f"Model initialized with embedding_dim={self.embedding_dim}, use_attention={use_attention}, use_se_block={use_se_block}, use_tcn={use_tcn}"
        )
    ...
    def predict(
        self, input_values: torch.Tensor, threshold: float = 0.5
