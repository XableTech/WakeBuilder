...
class RealNegativeDataLoader:
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in as_completed(futures):
                ...
                if processed_files % 500 == 0:
                    print(
                        f"  Processed {processed_files}/{len(audio_files)} files, {chunk_index} chunks..."
                    )
    ...
    def load_samples(
        ...
        if categories:
            search_dirs = [
                self.negative_dir / cat
                for cat in categories
                if (self.negative_dir / cat).exists()
            ]
        ...
        print(
            f"  Loaded {sample_count} real negative samples from {len(audio_files)} files"
        )
...
class MassivePositiveAugmenter:
    ...
    def _add_noise(self, audio: np.ndarray, snr_db: float) -> np.ndarray:
        ...
        noise = self._noise_loader.get_random_noise(
            self.target_duration, self.target_sr
        )
    ...
    def generate_samples_split(
        ...
        self._provider_train_targets = {
            p: per_provider_train_target for p in available_providers
        }
        self._provider_val_targets = {
            p: per_provider_val_target for p in available_providers
        }

        num_recordings = len(recordings)
        report_progress(
            f"  Generating {target_count} positive samples from {num_recordings} recordings...",
            31,
        )
        print(
            f"    Train: {train_size} ({train_rec_target} rec + {train_tts_target} TTS)"
        )
        print(
            f"    Val: {val_size} ({val_rec_target} rec + {val_tts_target} TTS with unseen voices)"
        )
        print(
            f"    TTS providers: {available_providers} ({per_provider_train_target} train / {per_provider_val_target} val each)"
        )
        ...
        if all_voices:
            print(
                f"    Piper TTS voices: {len(train_voices)} for training, {len(val_voices)} held-out for validation"
            )
        ...
        for rec_idx, (audio, sr) in enumerate(recordings):
            ...
            for i, (speed, pitch, volume, time_shift) in enumerate(aug_configs):
                ...
                if (
                    use_noise
                    and self._noise_loader
                    and self._noise_loader.num_samples > 0
                ):
            ...
            print(
                f"    Recording {rec_idx + 1}: {train_count} train, {val_count} val samples"
            )
        ...
        if use_tts and self._tts is not None:
            ...
            print(
                f"    TTS voices used (first pass): {len(voices_used)}/{len(train_voices)}"
            )

            # Second pass: add variations until target reached
            piper_train_target = self._provider_train_targets.get(
                "piper", train_tts_target
            )
            for voice in train_voices:
            ...
            print(
                f"    TTS val voices used (first pass): {len(val_voices_used)}/{len(val_voices)}"
            )
        ...
        if self._kokoro_tts is not None:
            ...
            kokoro_val_count = max(
                4, int(len(kokoro_voices) * 0.3)
            )  # 30% for validation
            kokoro_val_voices = set(kokoro_voices[:kokoro_val_count])
            kokoro_train_voices = set(kokoro_voices[kokoro_val_count:])

            report_progress(
                f"  Generating Kokoro TTS samples ({len(KOKORO_VOICES)} voices)...", 34
            )
            print(
                f"    Kokoro voices: {len(kokoro_train_voices)} train, {len(kokoro_val_voices)} val (unseen)"
            )
            print(
                f"    Speeds: {KOKORO_SPEED_VARIATIONS}, Volumes: {KOKORO_VOLUME_VARIATIONS} dB"
            )
            ...
            for voice_id in kokoro_train_voices:
                ...
                for speed in KOKORO_SPEED_VARIATIONS:  # [1.0, 1.5]
                    ...
                    try:
                        ...
                        voice_info = KOKORO_VOICES.get(
                            voice_id, ("Unknown", "unknown", "unknown", "a")
                        )
        ...
        if self._coqui_tts is not None:
            ...
            report_progress(
                f"  Generating Coqui TTS samples ({len(COQUI_MODELS)} models)...", 38
            )
            coqui_train_count = 0
            coqui_val_count = 0
            ...
            for model_key in self._coqui_tts.model_keys:
                if (
                    coqui_train_count >= coqui_train_target
                    and coqui_val_count >= coqui_val_target
                ):
                    break
                ...
                for speaker in train_spk:
                    ...
                    for speed in COQUI_SPEED_VARIATIONS:
                        if coqui_train_count >= coqui_train_target:
                            break
                        try:
                            audio, _ = self._coqui_tts.synthesize(
                                wake_word, model_key=model_key, speaker=speaker
                            )
                            audio = self._pad_or_trim(audio)
                            max_val = np.abs(audio).max()
                            if max_val > 0.01:
            ...
            report_progress(
                f"    Coqui TTS: {coqui_train_count} train, {coqui_val_count} val samples",
                40,
            )
        ...
        if self._edge_tts is not None:
            ...
            report_progress(
                f"  Generating Edge TTS samples ({len(voices)} voices)...", 41
            )
            ...
            for voice in train_voices:
                ...
                for speed in EDGE_SPEED_VARIATIONS:
                    if edge_train_count >= edge_train_target:
                        break
                    try:
                        audio, _ = self._edge_tts.synthesize(
                            wake_word, voice=voice, speed=speed
                        )
                        audio = self._pad_or_trim(audio)
                        max_val = np.abs(audio).max()
                        if max_val > 0.01:
            ...
            for voice in val_voices:
                ...
                try:
                    audio, _ = self._edge_tts.synthesize(
                        wake_word, voice=voice, speed=1.0
                    )
                    audio = self._pad_or_trim(audio)
                    max_val = np.abs(audio).max()
                    if max_val > 0.01:
                    ...
                    if h not in self._seen_hashes:
                        ...
                        val_samples.append(
                            (
                                audio,
                                {"source": "edge_tts", "voice": voice, "split": "val"},
                            )
                        )
                        edge_val_count += 1
                except Exception:
            ...
            report_progress(
                f"    Edge TTS: {edge_train_count} train, {edge_val_count} val samples",
                43,
            )
        ...
        report_progress(
            f"  Total: {len(train_samples)} train, {len(val_samples)} val positive samples",
            44,
        )
                    ...
                    for volume in self.volume_variations:
                        for time_shift in self.time_shifts:
                            ...
                            if (
                                ...
                                for snr in self.noise_snr_levels[
                                    :3
                                ]:  # Limit noise variations
                                    if rec_samples >= recording_target:
                                        break
                                    if sample_count >= target_count:
                                        break
                            ...
                            for volume in self.volume_variations[:3]:
                                ...
                                if (
                                    ...
                                    noisy = self._add_noise(
                                        aug, random.choice(self.noise_snr_levels)
                                    )
                                    noisy_hash = compute_audio_hash(noisy)
                                    if noisy_hash not in self._seen_hashes:
                                        self._seen_hashes.add(noisy_hash)
