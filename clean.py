...
CACHE_DIRS = {
    ...
    "coqui": {
        ...
        "path": Path(os.environ.get("LOCALAPPDATA", Path.home() / ".local" / "share"))
        / "tts",
...
def show_status() -> None:
    ...
    for key, info in CLEANABLE_DIRS.items():
        ...
        if path.exists():
            ...
            try:
                ...
                pass
            print(
                f"    {Symbols.BULLET} {info['description']}: {Colors.CYAN}{size}{Colors.RESET}"
            )
        else:
            print(
                f"    {Symbols.BULLET} {info['description']}: {Colors.DIM}(not created){Colors.RESET}"
            )
    ...
    for key, info in CACHE_DIRS.items():
        ...
        if path.exists():
            ...
            print(
                f"    {Symbols.BULLET} {info['description']}: {Colors.DIM}(not found){Colors.RESET}"
            )
...
Examples:
  ...
  python clean.py --status     Show disk usage only
    ...
    parser.add_argument("--all", action="store_true", help="Remove all data and caches")
    parser.add_argument(
        "--venv", action="store_true", help="Remove virtual environment"
    )
    parser.add_argument("--voices", action="store_true", help="Remove Piper TTS voices")
    parser.add_argument("--data", action="store_true", help="Remove training data")
    parser.add_argument("--models", action="store_true", help="Remove trained models")
    parser.add_argument(
        "--recordings", action="store_true", help="Remove user recordings"
    )
    parser.add_argument(
        "--cache", action="store_true", help="Remove HuggingFace and PyTorch caches"
    )
    parser.add_argument(
        "--status", action="store_true", help="Show disk usage status only"
    )
