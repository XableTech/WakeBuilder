...
def check_python_version() -> Tuple[bool, str]:
    ...
    if current >= required:
        ...
        return False, f"Python {current[0]}.{current[1]} is too old (required: {required[0]}.{required[1]}+)"
...
def check_in_venv() -> bool:
    ...
    return (
        ...
        os.environ.get('VIRTUAL_ENV') is not None  # explicit env var
    )
...
def get_pytorch_index(cuda_version: Optional[str]) -> Tuple[str, str]:
    ...
    # Default to latest
    return PYTORCH_CUDA_INDEXES[DEFAULT_CUDA_VERSION], f'CUDA {DEFAULT_CUDA_VERSION} (default)'
...
def install_dependencies(pkg_manager: str, pytorch_index: str, pytorch_label: str, step_num: int = 5) -> bool:
    ...
    code, stdout, _ = run_command(
        [str(venv_python), '-c', 'import uvicorn; import torch; import transformers; print("OK")'],
        capture=True
    )
    ...
    if code == 0 and 'OK' in stdout:
        ...
        code2, stdout2, _ = run_command(
            [str(venv_python), '-c', 'import torch; print("CUDA" if torch.cuda.is_available() else "CPU")'],
            capture=True
        )
    ...
    if code != 0:
        print_warn(f"PyTorch CUDA installation warning: {stderr[:100] if stderr else 'unknown'}")
    ...
    if pkg_manager == 'uv':
        ...
        if code != 0:
            ...
            code, _, stderr = run_command([
                'uv', 'pip', 'install', '--python', str(venv_python), '-r', 'pyproject.toml'
            ])
    else:
        # Use pip
        code, _, stderr = run_command([
    ...
    if code == 0:
        ...
        print_fail(f"Dependency installation failed: {stderr[:100] if stderr else 'unknown'}")
        return False
...
def download_tts_models(step_num: int = 6) -> bool:
    ...
    if len(piper_voices) >= 80:
        print_success(f"Piper TTS voices already downloaded ({len(piper_voices)} voices)")
    else:
        ...
        if download_script.exists():
            ...
            if code == 0:
                ...
                print_warn(f"Some Piper voices may have failed: {stderr[:100] if stderr else 'check logs'}")
        else:
...
def verify_installation(step_num: int = 7) -> bool:
    ...
    checks = [
        ("Python version", f"import sys; assert sys.version_info >= {MIN_PYTHON_VERSION}"),
...
def run_server(host: str = '0.0.0.0', port: int = 8000) -> None:
    ...
    cmd.extend([
        'src.wakebuilder.backend.main:app',
        '--host', host,
        '--port', str(port)
    ])
...
Available CUDA versions: {cuda_versions}
    ...
    parser.add_argument('--check', action='store_true', 
                        help='Verify environment and dependencies only')
    parser.add_argument('--install', action='store_true',
                        help='Install dependencies only')
    parser.add_argument('--download', action='store_true',
                        help='Download TTS models only')
    parser.add_argument('--run', action='store_true',
                        help='Run server only (skip checks)')
    ...
    if args.run:
        ...
        if not get_venv_python().exists():
            print_fail("Virtual environment not found. Run 'python run.py --install' first.")
            return 1
    else:
    ...
    if args.install or (not args.check and not args.run and not args.download):
        ...
        if already_installed:
            ...
            if interactive and not prompt_yn(f"Install dependencies ({pytorch_label})?"):
                print_info("Skipping dependency installation")
            else:
                if not install_dependencies(pkg_manager, pytorch_index, pytorch_label, step_num=4):
                    return 1
    ...
    if args.download or (not args.check and not args.run and not args.install):
        ...
        voices_dir = SCRIPT_DIR / 'tts_voices'
        piper_voices = list(voices_dir.glob('*.onnx')) if voices_dir.exists() else []
        already_downloaded = len(piper_voices) >= 80  # At least 80 of 87 expected voices
        
        if already_downloaded:
            print_header("Step 5: TTS Models")
            print_success(f"TTS voices already downloaded ({len(piper_voices)} Piper voices)")
        else:
    ...
    if args.check or (not args.run):
        if interactive and not args.check:
            ...
            if not verify_installation(step_num=6):
                if args.check:
                    return 1
                print_warn("Some verification checks failed, but attempting to continue...")
    ...
    if args.check or args.install or args.download:
        ...
        print_info(f"Start the server with: python run.py --run")
        print_info(f"Or: python run.py --auto")
        return 0
