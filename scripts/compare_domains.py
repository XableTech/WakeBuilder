...
from wakebuilder.audio.real_data_loader import (
...
def get_model_confidence(
    model, feature_extractor, audio: np.ndarray, sr: int = 16000
) -> float:
...
def main():
    ...
    try:
        ...
        print(
            f"Spec stats: min={spec.min():.2f}, max={spec.max():.2f}, mean={spec.mean():.2f}, std={spec.std():.2f}"
        )
    ...
    try:
        ...
        for audio, audio_metadata in augmenter.generate_samples(
            ...
            print(
                f"  [{audio_metadata.get('quality_aug', 'unknown')}] spec_mean={spec.mean():.2f}, conf={conf:.4f}"
            )
            count += 1
            if count >= 5:
                break
    ...
    neg_confidences = []
    count = 0
    for audio, audio_metadata in neg_loader.load_from_cache(
        max_samples=5, shuffle=True
    ):
        spec = preprocessor.process_audio(audio, 16000)
        conf = get_model_confidence(model, feature_extractor, audio, 16000)
...
the model may have learned TTS-specific features instead of the wake word.
    ...
    if tts_confidences:
        ...
        print(
            f"  Above threshold: {sum(1 for c in tts_confidences if c > threshold)}/{len(tts_confidences)}"
        )
    ...
    if neg_confidences:
        ...
        print(
            f"  False positives: {sum(1 for c in neg_confidences if c > threshold)}/{len(neg_confidences)}"
        )
