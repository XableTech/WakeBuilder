                ...
                if model_file.exists():
                    ...
                    if metadata_file.exists():
                        try:
                            ...
                            model_info["wake_word"] = metadata.get(
...
def list_models(models: list[dict]) -> None:
    ...
    for i, model in enumerate(models, 1):
        ...
        print(
            f"  {i}. {model['name']:20s} {category_badge:10s} wake_word='{model['wake_word']}'"
        )
    ...
    try:
        ...
        if not selection:
            ...
            models_sorted = sorted(
                models, key=lambda m: m["path"].stat().st_mtime, reverse=True
            )
...
def analyze_embeddings(
    ...
    for word in similar_words[:20]:  # Top 20 similar words
        ...
        if word_embeddings:
            ...
            # Cosine similarity
            sim = np.dot(wake_mean, word_mean) / (
                np.linalg.norm(wake_mean) * np.linalg.norm(word_mean)
            )
...
def test_model_predictions(
    ...
    for word in similar_words[:15]:  # Top 15 similar words
        ...
        if word_results["total"] > 0:
            ...
            print(
                f"  {word:20s} FP={fp_rate:.0%} avg={avg_score:.4f} max={max_score:.4f} {status}"
            )
    ...
    print(
        f"False positive rate on similar words: {total_fp}/{total_similar} ({total_fp/max(total_similar,1):.1%})"
    )

    if false_positives:
        print("\nFalse positives (detected as wake word):")
        for word, voice, score in sorted(
            false_positives, key=lambda x: x[2], reverse=True
        )[:10]:
...
Examples:
  ...
  python scripts/diagnose_model.py --model models/custom/jarvis/model.pt
    ...
    wake_word = (
        args.wake_word
        or checkpoint.get("wake_word")
        or model_info.get("wake_word", "unknown")
    )
    threshold = (
        args.threshold
        or checkpoint.get("threshold")
        or model_info.get("threshold", 0.5)
    )
    ...
    results = test_model_predictions(
        model, feature_extractor, tts, wake_word, similar_words, threshold
    )
...
"""
        ...
        print(
            "[OK] Model appears to be discriminating well between wake word and similar words"
        )
